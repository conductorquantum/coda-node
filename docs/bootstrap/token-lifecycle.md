...
Bootstrap tokens are one-time-use credentials created by operators in
...
| **Redeemed** | `redeemed_at IS NOT NULL` — consumed during a successful bootstrap. |
| **Revoked** | `revoked_at IS NOT NULL` — manually invalidated by an operator. |
| **Expired** | `expires_at <= now()` — past its time-to-live. |

Only active tokens can be used for bootstrap. The cloud returns
...
2. Computes `SHA-256(token)` and looks it up via the
   `lookup_bootstrap_token` database function.
3. If not found in the active view, queries `qpu_bootstrap_tokens`
   directly to determine the specific failure reason.
4. Returns the full token row (including `config` and `qpu_id`).

## Token Redemption

Tokens are redeemed atomically inside the
...
The `ON CONFLICT (id) DO UPDATE` clause means re-bootstrapping the same
...
The node receives the bootstrap token via:
...
After a successful bootstrap, credentials are persisted to disk and the
