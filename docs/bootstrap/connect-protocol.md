...
both first-time bootstrap and JWT-based reconnect. The cloud
...
|---|---|---|
| Bootstrap | Opaque string (not a JWT) | First run with `CODA_SELF_SERVICE_TOKEN` |
...
|---|---|---|---|
| `machine_fingerprint` | `string` | Recommended | Stable host identifier (`{hostname}-{mac_address}`). Generated by the node if not set. Stored on first bootstrap and verified on subsequent reconnects. |
| `opx_host` | `string` | Yes | OPX controller hostname. Falls back to token config if absent. |
| `opx_port` | `int` | Yes | OPX controller port (1–65535). Falls back to token config if absent. |

### Node-Side Request Construction

**Bootstrap** (`fetch_self_service_bundle` in `vpn/service.py`):
...
| Field | Bootstrap | Reconnect |
|---|---|---|
| `jwt_private_key` | Present (new keypair generated) | Absent (node already has it) |
...
## Cloud-Side Bootstrap Ordering

The cloud performs these steps atomically during bootstrap:
...
4. Redeem bootstrap token via `prepare_qpu_device_from_bootstrap_token`.
...
provisioning fails, and stale JWT keys are always cleaned up on failure.
