...
|---|---|---|---|
| `machine_fingerprint` | `string` | Recommended | Stable host identifier (`{hostname}-{mac_address}`). Generated by the node if not set. Stored on first run and verified on subsequent reconnects. |
